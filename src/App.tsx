...
function PacingStatistics({ workouts, targetFilter }: { workouts: Workout[]; targetFilter: RepTarget | 'all' }) {
  ...
  return <section className="time-statistics pacing-statistics" aria-labelledby="pacing-statistics-title"><div className="section-heading"><div><p className="eyebrow">Pacing</p><h2 id="pacing-statistics-title">Ritmo e recuperação</h2></div><span className="chart-unit">{stats.count} treino(s) guiado(s)</span></div><div className="time-stat-grid"><article><h3>Execução</h3><dl><div><dt>Tempo ativo</dt><dd>{formatDuration(stats.activeSeconds)}</dd></div><div><dt>Ritmo ativo</dt><dd>{formatDuration(Math.round(stats.activeSeconds / stats.reps))} <small>/ rep.</small></dd></div><div><dt>Ritmo total</dt><dd>{formatDuration(Math.round(stats.totalSeconds / stats.reps))} <small>/ rep.</small></dd></div></dl></article><article><h3>Recuperação</h3><dl><div><dt>Descanso real</dt><dd>{formatDuration(stats.restSeconds)}</dd></div><div><dt>Descanso planejado</dt><dd>{formatDuration(stats.plannedRestSeconds)}</dd></div><div><dt>Diferença</dt><dd>{formatDuration(Math.abs(stats.restSeconds - stats.plannedRestSeconds))} <small>{stats.restSeconds > stats.plannedRestSeconds ? 'a mais' : 'a menos'}</small></dd></div></dl></article></div></section>
}

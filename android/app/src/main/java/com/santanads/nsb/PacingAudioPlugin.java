...
public class PacingAudioPlugin extends Plugin {
    ...
    @PluginMethod public void signal(PluginCall call) {
        send(new Intent(getContext(), PacingAudioService.class).setAction(PacingAudioService.SIGNAL).putExtra("kind", call.getString("kind", "set")).putExtra("volume", call.getInt("volume", 100))); call.resolve();
    }
    @PluginMethod public void schedule(PluginCall call) {
        ...
        send(new Intent(getContext(), PacingAudioService.class).setAction(PacingAudioService.SCHEDULE).putExtra("delays", delays).putExtra("kinds", kinds).putExtra("volume", call.getInt("volume", 100))); call.resolve();
    }
